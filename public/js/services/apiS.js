...
angular.module('workingHoursTrello').service('apiS', function($http){
    ...
    this.getBoardMembers = function(key, token) {
      return  $http.get(`https://api.trello.com/1/boards/5ba38efef50b8979566922d0/members?key=${key}&token=${token}`);
    ...
    this.getBoardLists = function(key, token) {
      return $http.get(`https://api.trello.com//1/boards/5ba38efef50b8979566922d0/lists?key=${key}&token=${token}`);
    ...
    this.getBoardCards = function(key, token) {
      return $http.get(`https://api.trello.com/1/boards/5ba38efef50b8979566922d0/cards?key=${key}&token=${token}`);
    ...
    this.calendarBoardLists = function(key, token) {
      return $http.get(`https://api.trello.com//1/boards/5c6cb6171cf49a5579a42e7d/lists?key=${key}&token=${token}`);
