...
angular.module('workingHoursTrello').service('apiS', function($http){
  ...
  var url_board = window.location.href;
  var url_board_matches = url_board.match(/https\:\/\/trello\.com\/b\/[a-zA-Z0-9]+\//)
  var url_basic = url_board_matches[0];
  var url_report = url_basic+'reports.json';

  $http.get('url_report').then(function(board_data){ board_id = board_data.id; });
